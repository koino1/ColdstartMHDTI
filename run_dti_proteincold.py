...
from model.MHGNN_dti import MHGNN_lp
import math
...
def run_model(args):
    # 读取基础数据
    type_mask = np.load(os.path.join(args.data_dir, 'processed_coldstart_protein', 'node_types.npy'))
    ...
    train_pos = _read_pairs(os.path.join(split_dir, 'train_target_coldstart.json'))
    train_neg = _read_pairs(os.path.join(split_dir, 'train_target_coldstart_neg.json'))
    valid_pos = _read_pairs(os.path.join(split_dir, 'valid_target_coldstart.json'))
    valid_neg = _read_pairs(os.path.join(split_dir, 'valid_target_coldstart_neg.json'))
    test_pos  = _read_pairs(os.path.join(split_dir, 'test_target_coldstart.json'))
    test_neg  = _read_pairs(os.path.join(split_dir, 'test_target_coldstart_neg.json'))
...
def parser():
    ...
    ap.add_argument('--save_dir',
                    default='./results_dpp/{}/repeat{}/LLM_Proteinstart_trans_1114_改变投影层_固定学习率/neg_times{}_{}_{}_{}_num_head{}_hidden_dim{}_batch_sz{}_LLM_{}'
                            '_predictor_{}',
