...
def build_and_cache_for_split(
    ...
    save_adjM_npz=False, visible_dp_idx_for_eval=None,
):
    # 1) 只在训练集保留 D–P 正边；验证/测试不写入正边
    fold_drug_protein = np.zeros_like(drug_protein_full, dtype=drug_protein_full.dtype)
    # Only keep D–P positives for TRAIN split; keep zeros for valid/test
    ...
    if split_name in ('valid', 'test') and (visible_dp_idx_for_eval is not None) and (visible_dp_idx_for_eval.size > 0):
        d_tr, p_tr = visible_dp_idx_for_eval[0], visible_dp_idx_for_eval[1]
        fold_drug_protein[d_tr, p_tr] = drug_protein_full[d_tr, p_tr]

    # 2) 拼异质图邻接（仅 D–P 用 fold，其它关系用全量）
    adjM = get_adjM(
        drug_drug, fold_drug_protein, drug_disease, drug_sideEffect,
        protein_protein, protein_disease,
        num_drug, num_protein, num_disease, num_se
    )

    # === NEW: 基于当前 split 的 adjM 生成一跳邻居字典，并暴露为全局变量，供 get_metapath 使用 ===
    ...
    dp_block = adjM[:num_drug, num_drug:num_drug + num_protein].astype(bool)

    if split_name == 'train':
        if dp_idx is not None and dp_idx.size > 0:
            mask_train = np.zeros_like(dp_block, dtype=bool)
            mask_train[dp_idx[0], dp_idx[1]] = True
            leak = (dp_block & ~mask_train).sum()
            assert leak == 0, f"[{split_name}] D–P 出现非本 split 的边：{leak}"
        else:
            assert dp_block.sum() == 0, f"[{split_name}] 训练集却没有正边？"
    else:
        if visible_dp_idx_for_eval is not None and visible_dp_idx_for_eval.size > 0:
            mask_visible = np.zeros_like(dp_block, dtype=bool)
            mask_visible[visible_dp_idx_for_eval[0], visible_dp_idx_for_eval[1]] = True
            leak = (dp_block & ~mask_visible).sum()
            assert leak == 0, f"[{split_name}] 发现非训练集的 D–P 边（疑似泄露）：{leak}"
        else:
            # 若未传 visible，可退化为“必须全 0”（与旧逻辑兼容）
            assert dp_block.sum() == 0, f"[{split_name}] 非训练集的 D–P 应为全 0"


    # 3) 生成该 split 的 metapath 缓存（与原逻辑一致，只是目录换成 split）
    ...
    for i, metapaths in enumerate(expected_metapaths):  # i=0 Drug端, i=1 Protein端
        for metapath in metapaths:
            ...
            if os.path.isfile(npy_path):
                ...
                # 现在 get_metapath 能读到“当前 split”的全局邻居字典了
...
if __name__ == '__main__':
    ...
    train_idx = load_pairs_to_idx2(os.path.join(data_dir, 'train_pos_1111.json'))  # shape (2, N_train)
    valid_idx = load_pairs_to_idx2(os.path.join(data_dir, 'valid_pos_1111.json'))  # shape (2, N_valid)
    test_idx = load_pairs_to_idx2(os.path.join(data_dir, 'test_pos_1111.json'))  # shape (2, N_test)
    ...
    for counter in range(num_repeats):
        ...
        _adj_valid = build_and_cache_for_split(
            ...
            protein_protein=protein_protein, protein_disease=protein_disease,
            num_drug=num_drug, num_protein=num_protein, num_disease=num_disease, num_se=num_se,
            expected_metapaths=expected_metapaths, counter=counter, save_prefix=save_prefix, visible_dp_idx_for_eval=train_idx
        )
