...
from utils.data_casestudy import load_data, fold_train_test_idx, mydataset, collate_fc, get_features
...
def run_model(args):
    # 读取基础数据
    type_mask = np.load(os.path.join(args.data_dir, 'processed_case_study_ESR1', 'node_types.npy'))
    ...
    train_pos = _read_pairs(os.path.join(split_dir, 'train_case_study_ESR1.json'))
    train_neg = _read_pairs(os.path.join(split_dir, 'train_case_study_ESR1_neg.json'))
    valid_pos = _read_pairs(os.path.join(split_dir, 'valid_case_study_ESR1.json'))
    valid_neg = _read_pairs(os.path.join(split_dir, 'valid_case_study_ESR1_neg.json'))
    test_pos  = _read_pairs(os.path.join(split_dir, 'test_case_study_ESR1.json'))
    test_neg  = _read_pairs(os.path.join(split_dir, 'test_case_study_ESR1_neg.json'))
...
def parser():
    ...
    ap.add_argument('--save_dir',
                    default='./results_dpp/{}/repeat{}/LLM_datasetB_casestudy_ESR1/neg_times{}_{}_{}_{}_num_head{}_hidden_dim{}_batch_sz{}_LLM_{}'
                            '_predictor_{}',
