...
def run_model(args):
    # 读取基础数据
    type_mask = np.load(os.path.join(args.data_dir, 'processed', 'node_types.npy'))
    ...
    train_pos = _read_pairs(os.path.join(split_dir, 'train_pos_1111.json'))
    train_neg = _read_pairs(os.path.join(split_dir, 'train_neg_1111.json'))
    valid_pos = _read_pairs(os.path.join(split_dir, 'valid_pos_1111.json'))
    valid_neg = _read_pairs(os.path.join(split_dir, 'valid_neg_1111.json'))
    test_pos = _read_pairs(os.path.join(split_dir, 'test_pos_1111.json'))
    test_neg = _read_pairs(os.path.join(split_dir, 'test_neg_1111.json'))
...
def parser():
    ...
    ap.add_argument('--save_dir',
                    default='./results_dpp/{}/repeat{}/LLM_Warmstart_transgraph_1113_改变投影层_固定学习率/neg_times{}_{}_{}_{}_num_head{}_hidden_dim{}_batch_sz{}_LLM_{}'
                            '_predictor_{}',
