...
def build_and_cache_for_split(
    ...
    if split_name == 'train':
        if dp_idx is not None and dp_idx.size > 0:
            ...
            assert leak == 0, f"[{split_name}] D–P block 有 {leak} 条非本 split 的边，疑似信息泄露"
        else:
...
if __name__ == '__main__':
    ...
    train_idx = load_pairs_to_idx2(os.path.join(data_dir, 'train_case_study_EGFR.json'))  # shape (2, N_train)
    valid_idx = load_pairs_to_idx2(os.path.join(data_dir, 'valid_case_study_EGFR.json'))  # shape (2, N_valid)
    test_idx = load_pairs_to_idx2(os.path.join(data_dir, 'test_case_study_EGFR.json'))  # shape (2, N_test)

    save_prefix = data_dir + '/processed_case_study_EGFR/'
